...
勝手に推測して公開するより、出ていないほうが安全だからです。

未分類商品は、**完全な商品データと画像を保持したまま**
`_app/storage/pending/` に保留されます（Web非公開領域です）。
これにより、管理者が管理画面「Shop Sync Status」でカテゴリを選ぶと、
**次回の同期を待たずにその場で公開**できます。

| 操作 | 結果 |
|---|---|
| カテゴリを設定 | その商品だけ即時公開。画像も公開領域へコピー。`categoryOverridden = true` |
| 何もしない | 未分類一覧に残り、公開されない |
| 手動設定を解除 | その場で自動分類をやり直す。判定できれば公開継続、できなければ公開停止して未分類へ戻す |

公開反映は `products.json` の atomic な差し替え（一時ファイル → `rename`）で行うため、
途中で失敗しても商品データが壊れることはありません。
pending領域は同期のたびに作り直されるので、分類済みになった商品や
minneから消えた商品が残り続けることもありません。
